...
(use-package counsel
  ...
  (defvar my-ivy-preview-buffers nil)

  (defun my-ivy-track-previews (orig-fun &rest args)
    "Keep track of buffers opened during ivy-call for later cleanup."
    (let ((before-buffers (buffer-list)))
      (apply orig-fun args)
      (let ((after-buffers (buffer-list)))
        (setq my-ivy-preview-buffers 
              (append my-ivy-preview-buffers 
                      (cl-set-difference after-buffers before-buffers))))))

  (defun my-ivy-cleanup-previews ()
    "Kill buffers opened during preview that are not the current buffer."
    (let ((current (current-buffer)))
      (dolist (buf my-ivy-preview-buffers)
        (when (and (buffer-live-p buf) (not (eq buf current)))
          (kill-buffer buf)))
      (setq my-ivy-preview-buffers nil)))

  ;; Track buffers opened via C-M-n or auto-calling
  (advice-add 'ivy-call :around #'my-ivy-track-previews)

  ;; Clean up when the minibuffer closes
  (add-hook 'minibuffer-exit-hook #'my-ivy-cleanup-previews)
