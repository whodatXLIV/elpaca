...
- [X] Use tabbar and window managers to fix the output.
...
- [ ] use popper to be able to handle the outputs and errors and stuff
...
(use-package code-cells
  ...
  (defun seth-add-time-stamp-after-execute (&optional var)
    (interactive)
    (unless var (setq var nil))
    (unless (boundp 'seth-jupyter-execution-count) (setq-local seth-jupyter-execution-count 1))
    (let (
  ...
  (defun seth-restart-jupyter-execution-count (&rest arg)
    (setq-local seth-jupyter-execution-count 1)
    )
  ;; add below to keep track of what has been run
  (add-hook 'python-mode-hook (lambda ()
                                (setq-local seth-jupyter-execution-count 1)))
  (advice-add 'jupyter-repl-restart-kernel :before #'seth-restart-jupyter-execution-count)
  (advice-add 'jupyter-run-repl :before #'seth-restart-jupyter-execution-count)
  ...
  :bind (
         ...
         ("C-c C-c" . seth-add-time-stamp-after-execute)
         ("C-<return>" . (lambda() (interactive) (seth-add-time-stamp-after-execute 1)))
         ("M-p" . code-cells-backward-cell)
         ("M-n" . code-cells-forward-cell)
         ;; ("C-c % B" . code-cells-move-cell-up)
         ;; ("C-c % F" . code-cells-move-cell-down)
         ("C-c ;" . code-cells-comment-or-uncomment)
         ("C-c a" . code-cells-mark-cell)
         ;; add clear stuff
...
(use-package vundo
  ...
  :delight
  :custom
  (undo-limit         (* 96 1024 1024)) ;  96 MiB. The change group at which this size is exceeded is the last one kept.
  (undo-strong-limit (* 128 1024 1024)) ; 128 MiB. The change group at which this size is exceeded is discarded itself (along with all older change groups). There is one exception: the very latest change group is only discarded if it exceeds ‘undo-outer-limit’.
