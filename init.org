...
(eval-after-load 'smime
  '(progn
     (message "DEBUG: smime loaded, applying caching config.")
                                          ...
                                          (let* ((from (mu4e-message-field msg :from))
                                            ...
                                            (unwind-protect
                                                (progn
                                                  ...
                                                  ;; Directly verify and extract certs from the original message file
                                                  ;; This command expects a signed message, not necessarily encrypted.
                                                  (let ((command (format "openssl cms -verify -inform SMIME -in %s -noverify -certsout %s"
                                                                         (shell-quote-argument original-message-path) ; Use original raw message file
                                                                         (shell-quote-argument filename-crt))))
                                                    (message "DEBUG: Running verify command for import...")
                                                    (if (zerop (shell-command command))
                                                      ...
                                                      (message "Failed to extract certificate. Is the email S/MIME signed? (Was it perhaps only encrypted?)"))))
...
;; 	          )
;;   :init
;;   (require 'goto-addr) ; Ensures goto-address-url-regexp is defined
;;   :config
