...
(use-package company
  :delight
  :defer t
  :ensure company-spell
  :bind* ("C-<tab>" . company-other-backend)
  :hook ((prog-mode org-mode python-mode LaTeX-mode) . company-mode)
  :custom-face
...
the list is kept."
    (let ((tail list) last)
      (while (cdr tail)
        ...
        (let ((eq1 (substring-no-properties (car tail)))
              (eq2 (substring-no-properties (cadr tail))))
