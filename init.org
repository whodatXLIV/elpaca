...
(use-package company
  ...
  ;; TODO add transform function delete consecutuve that splits strings at parens?
  )
...
;; TODO add in function to truly toggle quickhelp by setting nil to 0 and back
...
;; TODO figure out why the windows spilt so stupidly
...
(use-feature python
  ...
  (defun my-python-company-mode-hook ()
    (setq-local company-backends
                '((company-yasnippet company-dabbrev-code :with company-capf)
                  company-files)))
...
#+begin_src emacs-lisp :lexical t :tangle yes
;; BUG going left when no files or project does not go to views
  ...
  (defun switch-highlighted-tab-or-buffer (&optional direction)
    ...
    (let* ((tablist (tab2-get-tabs))
      ...
      (cond ((not isGroup)
            ...
            (isGroup
             (if direction
                 (funcall (cadr (nth current-index (tab-rotate-list-right name-select)))) ;; change to selected tab (rotated)
             ...
             (let* (
               ...
               (tab-line-select-tab-buffer (nth buffer-current-index buffer-select) (selected-window)) ;; go to buffer
               )
             (when (not (window-parameter nil 'tab-line-views)) ;; don't go up level if in views
               (tab2-select-filter-button t) ;; go up level
