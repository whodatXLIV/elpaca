  ...
  (add-to-list 'mu4e-view-actions
               '("Import S/MIME cert" . (lambda (msg)
                                          (let* ((from (mu4e-message-field msg :from))
                                                 ...
                                                 (filename (and email (expand-file-name (concat email ".crt") cert-dir)))
                                                 (original-message-path (mu4e-message-field msg :path)) ; Path to the raw message file
                                                 )
                                            (unless (and email original-message-path cert-dir filename)
                                                  ...
                                                  (let ((command (format "openssl cms -verify -inform SMIME -in %s -noverify -certsout %s"
                                                    ...
                                                    (if (zerop (shell-command command))
                                                        (progn
                                                          ...
                                                          (message "S/MIME certificate for %s imported." email))
