...
(eval-after-load 'smime
  '(progn
     ...
     ;; --- S/MIME Passphrase Caching ---
     (defvar my/smime-passphrase nil
       "Cache for the S/MIME passphrase for the current Emacs session.")

     (defun my/get-smime-passphrase ()
       "Get the S/MIME passphrase, prompting if not already cached."
       (unless my/smime-passphrase
         (setq my/smime-passphrase (read-passwd "S/MIME Passphrase (caching): ")))
       my/smime-passphrase)

     (defun my/smime-add-passphrase-advice (old-fn operation &rest options)
       "Advice for `smime-openssl` to add the cached passphrase for decryption."
       (if (eq operation '-decrypt)
           (let ((pass (my/get-smime-passphrase)))
             (if (and pass (> (length pass) 0))
                 (apply old-fn operation (append options (list "-passin" (concat "pass:" pass))))
               (apply old-fn operation options)))
         (apply old-fn operation options)))

     (advice-add 'smime-openssl :around #'my/smime-add-passphrase-advice)))
...
(use-package mu4e
  ...
  (defun my-mu4e-ask-crypto ()
    "Ask whether to sign or encrypt the message."
    (unless (message-mail-p)
      (let ((sign (y-or-n-p "Sign message? "))
            (encrypt (y-or-n-p "Encrypt message? ")))
        (if (and sign encrypt)
            (mml-secure-message-sign-and-encrypt-pgpmime)
          (if sign (mml-secure-message-sign-pgpmime))
          (if encrypt (mml-secure-message-encrypt-pgpmime))))))

  (add-hook 'message-send-hook 'my-mu4e-ask-crypto)
