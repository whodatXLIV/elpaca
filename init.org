...
***** how to open python [0/5]
- [ ] first need to start kernel (how to know which one to start)
- [ ] that same kernel needs to start eglot (must get sent to eglot in parallel)
- [ ] need to name that kernel the file name (so that kernels are unique)
- [ ] need to associate the kernel with the buffer using associate-buffer
- [ ] Use tabbar and window managers to fix the output. capture output and organize somehow
...
(setq jupyter-eval-use-overlays t) I like this
