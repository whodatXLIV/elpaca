...
(use-package mu4e
  ...
  (with-eval-after-load 'shr
    (setq shr-use-colors nil        ; Don't let HTML dictate text/background colors
          shr-use-fonts nil         ; Stick to your default Emacs font (monospace)
          shr-width 80              ; Restrict the width to 80 characters for readability
...
(use-package mu4e-alert
  ...
  ;; Define which maildirs to monitor for notifications (optional, monitors all unread by default)
  ;; (setq mu4e-alert-interesting-maildirs '("/Focus/INBOX" "/Gmail/INBOX"))

  ;; Group notifications by Maildir so each account (Focus vs Gmail) gets its own notification
  (setq mu4e-alert-group-by :maildir)

  ;; ;; Custom formatter to make the separated notifications look nice
  ;; (setq mu4e-alert-grouped-mail-notification-formatter
  ;;       (lambda (mail-group all-mails)
  ;;         (let* ((mail-count (length mail-group))
  ;;                (first-mail (car mail-group))
  ;;                (maildir (plist-get first-mail :maildir))
  ;;                ;; Extract the account name (e.g. "Focus" or "Gmail") from the maildir path
  ;;                (account-name (if (string-match "^/\\([^/]+\\)" maildir)
  ;;                                  (match-string 1 maildir)
  ;;                                "Account"))
  ;;                (title (format "%s: %d New Message%s" 
  ;;                               account-name 
  ;;                               mail-count 
  ;;                               (if (> mail-count 1) "s" "")))
  ;;                (body (mapconcat
  ;;                       (lambda (mail)
  ;;                         (format "• %s: %s"
  ;;                                 (car-safe (car-safe (plist-get mail :from)))
  ;;                                 (truncate-string-to-width (plist-get mail :subject) 40 nil nil "...")))
  ;;                       (seq-take mail-group 5) "\n"))) ; Show max 5 messages in the banner
