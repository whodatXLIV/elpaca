...
*** python
first need to start kernel
that same kernel needs to start eglot
need to name that kernel the file name
need to associate the kernel with the buffer
...
add in a way to rewrite the code cells to have a timestamp

use popper to be able to handle the outputs and errors and stuff

Use tabbar and window managers to fix the output. New frame per output
...
#+LASTRUN: [2024-08-26 Mon 16:39:30] [2]
#+begin_src emacs-lisp :lexical t
(use-package neotree
  :delight
  :defer t
